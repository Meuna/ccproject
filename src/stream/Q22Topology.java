...
public class Q22Topology extends ConfigurableTopology  {

    public static void main(String[] args) throws Exception {
        ConfigurableTopology.start(new Q22Topology(), args);
    }
    ...
    protected int run(String[] args) throws Exception {
        ...
        return submit("q22", conf, builder);
    }
    ...
    public static class OriginDestDelayBolt implements IRichBolt {
        ...
        public void execute(Tuple tuple) {
            if (this.pseudoBatch && tuple.getSourceStreamId().equals("EOF")) {
                for (Map.Entry<String, Double> entry : this.odMeanDelay.entrySet()) {
                    String[] originDest = entry.getKey().split("_");
                    collector.emit(new Values(originDest[0], originDest[1], BigDecimal.valueOf(entry.getValue())));
                }
            } else {
        ...
        public void declareOutputFields(OutputFieldsDeclarer declarer) {
            declarer.declare(new Fields("origin", "dest", "delay"));
        }
